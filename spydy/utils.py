...
def class_dispatcher(user_provide_classname:str):
    if user_provide_classname.startswith("file:"):  # Deal with the class that provided by users 
        ...
        try:
            ...
            raise AssertionError("The method {!} you provided seems like has a wrong form.")
...
def get_value_from_moulde(*args, **kwargs):
    return get_class_from_moudle(*args, **kwargs)
...
def configs_assertion(configs):
    ...
    try:
        assert "run_mode" in configs["Globals"] and configs["Globals"]["run_mode"] in RUNMODES
    except:
        raise AssertionError("Parameter <run_mode> seems not be provides or run_mode not in {}".format(RUNMODES))
